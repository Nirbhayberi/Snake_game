...
def game_loop() -> None:
    ...
    if snake_obj.check_food_collision(food_obj.position):
        ...
        food_turtle.goto(food_obj.position)
        '''nsegment = turtle.Turtle() #adding tail segment
        nsegment.speed(0)
        nsegment.shape("square")
        nsegment.color(random.choice(["orange","black","green","white"]))
        nsegment.penup()
        nsegmentlst.append(nsegment)'''
    ...
    if snake_obj.GAME_OVER==True:
        ...
        #for i in nsegmentlst:
            #i.goto(snake_obj.window_size[0],snake_obj.window_size[1])
        #nsegmentlst.clear()
